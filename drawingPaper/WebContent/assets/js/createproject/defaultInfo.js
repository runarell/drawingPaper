...
imgInput.onchange = function (obj) {
  ...
  fileArr.forEach((file) => {
    ...
    fileReader.onload = function (e) {
      ...
      //이미지 리스트의 길이(사진의 수)
      document.getElementsByClassName("img-length")[0].innerHTML = "이미지 업로드(" + index + "/5)";
    };
...
function changeImg(obj) {
  ...
  fileReader.onload = function (e) {
    let imgSrc;
    imgSrc = e.target.result;
    obj.parentNode.parentNode.parentNode.parentNode.querySelector("img").src =
      imgSrc;
  };
...
function deleteImg(obj) {
  let target = obj.parentNode.parentNode.parentNode.parentNode.parentNode;
  target.remove();
  setNum();
}
//이미지 번호 부여
function setNum() {
  let temp = document.getElementsByClassName("img_list");
  let lastIdx = 0;
  for (let i = 0; i < temp.length; i++) {
    let span = temp.item(i).querySelector(".img_stepper > span");
    span.textContent = i + 1;
    lastIdx = i + 1;
  }
  document.querySelector(".img-length").textContent = "이미지 업로드(" + lastIdx + "/5)";
}
